/*
  ...
  character set in languages like Arabic, Chinese, Japanese, Korean, and
  Russian.  This gives consistent names to font files, no matter what their
  source.  The contents of the files are not changed, only the names in the
  file directory.  Don't use this program on system folders with installed
  fonts.

  To display all name tables inside a font, in all languages, see the FontNames
  Java application.  For more information on the internal format of font files,
  start with the following on-line references:
  ...
  --------------------------------------------
  FontRename4 is free software and has been released under the terms and
...
//  }
    ...
    outputText.setText(
      "\nRename OpenType and TrueType font files with their internal \"font"
      + "\nfull name\" plus an extension for the type (OTF, TTC, TTF).  The"
      + "\ncontents of the files are not changed, only file directory names."
      ...
      + "\nDon't touch system folders or installed fonts!"
...
/*
  ...
  For a list of possible character set names, as supported by Java 5.0 (1.5),
    ...
    try                           // catch file I/O errors
    {
      ...
      else
      {
        ...
        else if (signature == 0x74746366L) // "ttcf" for TrueType collection
        {
          ...
          /* As of 2017, nobody can agree on the correct file name extension
          for OpenType collection (OTC) font files.  Only Adobe appears to be
          producing OTC files, to introduce the idea.  (Apple may have some as
          system fonts for MacOS.)  Microsoft's official documentation says
          file names should be *.OTC but files are being renamed as *.TTC so
          that they work on MacOS and Windows 10.  OTC files currently have the
          same signature bytes as TTC files, and since the TTC file format is
          simply a container for internal OTF or TTF fonts, nothing prevents an
          OTC or TTC file from having a mixture of both OTF and TTF fonts!

          The code below checks only if the first internal font is marked as
          OpenType.  It does not check for PostScript (CFF) glyphs compared to
          TrueType glyphs.  It does not check if all remaining internal fonts
          are consistent with the first font. */

          if (fileSize >= (offsetTable + 4)) // should be safe, check anyway
          {
            ramFile.seek(offsetTable); // signature bytes, first internal font
            if ((((long) ramFile.readInt()) & INT_MASK) == 0x4F54544FL)
            {
              fileType = "otc";   // "OTTO" signature forces *.OTC file type
            }
          }
          else
          {
            /* Ignore errors if first Offset Table location is bad.  Invalid
            locations will be caught later, as will corrupt data. */
          }
        }
...
//  fileName = givenFile.getName(); // get name of caller's file, no path
    ...
    if (forceOpen                 // this option overrides actual file type
      || fileType.equals("otc")   // OpenType collection
